...
import {
  Plus, Calendar, Users, DollarSign, Wallet, Bell, Settings,
  ChevronRight, Clock, MapPin, CheckCircle, AlertCircle, ArrowUpRight,
  Gift, Heart, QrCode, LogOut, Star, TrendingUp, Zap
} from 'lucide-react';
...
function StatCard({ icon: Icon, label, value, sub, color, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`bg-white rounded-2xl p-5 border border-gray-100 hover:shadow-lg transition-all text-left w-full group ${onClick ? 'cursor-pointer' : 'cursor-default'}`}
    >
...
export default function DIYDashboard() {
  const { user, logout, triggerKyc } = useAuth();
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="fixed left-0 top-0 bottom-0 w-64 bg-gray-950 flex flex-col z-40 hidden lg:flex">
        ...
        <div className="px-6 py-5 border-b border-white/10">
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className="text-white font-bold">EventPark</span>
          </Link>
        </div>
        ...
        <div className="px-4 py-4 border-b border-white/10">
          <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 cursor-pointer">
            ...
            <div>
              ...
              <div className="text-gray-500 text-xs">Personal · Tier {user?.kycTier}</div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-grow px-3 py-4 space-y-1">
          {[
            { label: 'My Events', icon: Calendar, active: true, href: '/dashboard' },
            { label: 'Discover Vendors', icon: Star, href: '/discover/vendors' },
            { label: 'My Wishlist', icon: Gift, href: '/dashboard/wishlist' },
            { label: 'Wallet', icon: Wallet, href: '/wallet' },
            { label: 'Guest Lists', icon: Users, href: '/dashboard/guests' },
            ...
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${
                item.active ? 'bg-brand-600 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'
              }`}
            >
        ...
        {/* Wallet preview */}
        <div className="mx-3 mb-3 p-4 rounded-2xl bg-gradient-to-br from-brand-600 to-brand-800">
          ...
          <div className="text-brand-300 text-xs mt-0.5">₦{(user?.walletEscrow || 0).toLocaleString()} in escrow</div>
        ...
        <button
          ...
          className="flex items-center gap-3 mx-3 mb-4 px-3 py-2.5 rounded-xl text-gray-500 hover:text-white hover:bg-white/5 text-sm transition-all"
        >
      ...
      <div className="lg:pl-64 min-h-screen">
        ...
        <div className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-gray-100 px-6 py-3 flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-gray-900">Good morning, {user?.firstName} 👋</h1>
            <p className="text-xs text-gray-400">{activeEvent.title} · {activeEvent.daysLeft} days to go</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/events/create"
              className="flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold px-4 py-2 rounded-xl transition"
            >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex gap-3 flex-wrap">
            {myEvents.map(ev => (
              ...
              >
                <span className={`w-2 h-2 rounded-full ${ev.status === 'active' ? 'bg-green-500' : 'bg-yellow-400'}`} />
                {ev.title.split(' – ')[0]}
              </button>
            ))}
            <Link
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <StatCard icon={DollarSign} label="Budget used" value={`${budgetPct}%`} sub={`₦${(activeEvent.spent/1000000).toFixed(1)}M of ₦${(activeEvent.budget/1000000).toFixed(0)}M`} color="bg-orange-50 text-orange-500" onClick={() => {}} />
            <StatCard icon={Wallet} label="Wallet" value={`₦${((user?.walletBalance || 0)/1000).toFixed(0)}K`} sub="Available balance" color="bg-purple-50 text-purple-600" onClick={() => navigate('/wallet')} />
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-6">
                  ...
                  {activeEvent.vendors.map(v => (
                    <div key={v.name} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                      ...
                      <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${
                        ...
                        {v.status === 'confirmed' ? '✓ Confirmed' : v.status === 'pending_escrow' ? '⏳ Escrow pending' : '💬 Quoted'}
                      </span>
                    </div>
                  ))}
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-xs text-gray-400">
                    {activeEvent.todos.filter(t => t.done).length}/{activeEvent.todos.length} done
                  </span>
                </div>
                <div className="space-y-2">
                  {activeEvent.todos.map((todo, i) => (
                    <div key={i} className={`flex items-center gap-3 p-3 rounded-xl border transition ${todo.done ? 'border-green-100 bg-green-50' : 'border-gray-100 bg-gray-50'}`}>
                      ...
                      {todo.due && !todo.done && (
                        <span className="text-xs text-orange-500 font-medium">Due {todo.due}</span>
                      )}
                    </div>
                  ))}
                  <button className="w-full flex items-center gap-2 p-3 rounded-xl border border-dashed border-gray-200 text-gray-400 hover:border-brand-300 hover:text-brand-500 text-sm transition-all">
                    <Plus className="w-4 h-4" />
                    Add task
                  </button>
                </div>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                ...
                <div className="flex items-center justify-center mb-4">
                  <div className="relative w-28 h-28">
                    <svg className="w-full h-full -rotate-90" viewBox="0 0 36 36">
                      <circle cx="18" cy="18" r="15.9" fill="none" stroke="#f3f4f6" strokeWidth="2" />
                      <circle cx="18" cy="18" r="15.9" fill="none" stroke="#4f52e6" strokeWidth="2"
                ...
                <Link to="/dashboard/guests" className="mt-3 flex items-center justify-center gap-1 text-sm text-brand-600 font-semibold hover:underline">
                  Manage guest list <ChevronRight className="w-3.5 h-3.5" />
                </Link>
              </div>
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Link to="/dashboard/wishlist" className="text-xs text-brand-600 font-semibold">View all</Link>
                </div>
                <div className="space-y-3">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                ...
                <div className="space-y-2">
                  {[
                    { label: 'Invite collaborator', icon: Users, href: '/dashboard/collaborators' },
                    { label: 'Day-of check-in', icon: QrCode, href: '/checkin/e1' },
                    { label: 'Top up wallet', icon: Wallet, action: () => navigate('/wallet') },
                    { label: 'Browse vendors', icon: Star, href: '/discover/vendors' },
                  ].map(action => (
                    action.href ? (
                      <Link key={action.label} to={action.href} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-gray-50 transition-all text-sm text-gray-700 font-medium">
                      ...
                      <button key={action.label} onClick={action.action} className="w-full flex items-center gap-3 p-2.5 rounded-xl hover:bg-gray-50 transition-all text-sm text-gray-700 font-medium">
                        <div className="w-8 h-8 rounded-lg bg-brand-50 text-brand-600 flex items-center justify-center">
                          <action.icon className="w-4 h-4" />
                        </div>
                        {action.label}
